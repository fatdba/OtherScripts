def run_query(secrets_client, secret_arn, sql):
    try:
        ...
        if dbconn:
            ...
            status_code = -1
            return status_code, status  # Return only two values
        result = pgs.run_query_using_secrets(secret_client, secret_arn,sql)
    ...
    return status_code, status, result  # Return only two values
