...
def get_account_ids():
    ...
    try:
        ...
        while 'NextToken' in response:
            ...
            for each in response.get("Accounts"):
                if environment+"-" in each.get("Name").lower() or "-"+environment in each.get("Name").lower():
                    acct_ids.append(each.get("Id"))
    ...
    return acct_ids, error

def get_audit_roles_list(scan_id, audit_roles_list, result, dbname):
...
def lambda_handler(event, context):
    ...
    summary_table_parameter_list = ["ScanID", "AccountID", "RDSIdentifier", "SecretARN", "Summary"]
    ...
    if error != "":
        #If any error with summary list refer Comment above the summary_table_parameter_list
        summary_list.append({"ScanID":scan_id, "AccountID":"ALL", "RDSIdentifier": "NA", "SecretARN":"NA", "Summary":error})
    print(len(acct_ids))
    # acct_ids = ["472131731879"]
    
    # acct_ids_temp = ["472131731879", "589839611729"]
    # acct_ids.extend(acct_ids_temp)
    ...
    for acct_id in acct_ids:
        ...
        for each in rds_list:
            ...
            db_admin_secret_name='/secret/'+each["DBInstanceID"]+'/rds-password'
            db_admin_secret_name_list.append(db_admin_secret_name)
        ...
        while index < len(rds_secrets_list):
            ...
            if rds_secrets_list[index]['DBAdminSecretARN'] != "":
                ...
                #If query execution successfull runs the CIS sql scripts on target db for drifts.
                    ...
                    audit_roles_list = get_audit_roles_list(scan_id, audit_roles_list, result, dbname=rds_secrets_list[index]['DatabaseName'])
                    ...
                    if result:
                        print("result_type: ", type(result[0]))
