...
# Open the JSON file
with open('results_out.json', 'r') as f:
...
acct_ids = [entry['account_id'] for entry in data]
print(len(acct_ids))
...
def update_table(rds_list, parameter_list, table_name):
    ...
    status_code = run_query_using_secret(sql1)
...
def create_or_alter_table(parameter_list, table_name):
    ...
    status_code = run_query_using_secret(sql)
    # status_code,status=run_query(secrets_client,reporting_db_secret_arn,sql)
    ...
    if rds_summary_list == [] and len(rds_instances['DBInstances']) > 0:
        while index < len(rds_instances['DBInstances']):
            try:
                ...
                if 'DBClusterIdentifier' in rds_instances['DBInstances'][index]:
                    #If any error with summary list refer Comment above the summary_table_parameter_list
                    rds_summary_list.append({"ScanID":scan_id, "AccountID":acct_id, "RDSIdentifier": str(rds_instances['DBInstances'][index]['DBClusterIdentifier'].rsplit('-',1)[0]), "Secr
etARN":"NA", "Summary":error})
                else:
                    #If any error with summary list refer Comment above the summary_table_parameter_list
                    rds_summary_list.append({"ScanID":scan_id, "AccountID":acct_id, "RDSIdentifier": rds_instances['DBInstances'][index]['DBInstanceIdentifier'], "SecretARN":"NA", "Summary
":error})
...
def get_account_ids():
    ...
    try:
        ...
        org_client = boto3.client(
            'organizations', region_name="us-east-2",
...
#             result = run_query_using_secret(sql)
#         except Exception as e:
#             result = []
...
def generate_csv_and_pdf_reports_for_the_drift_tables(table_names_list, scan_id):
    """
    Function to gnereate CSV and pdf reports from the tables in drift reporting database
    secrets_client: Parameter to connect to Drift Reporting DB
    table_names_list: list of the all the drift tables to generate CSV and PDf reports from.
    ...
    s3_resource = boto3.resource('s3')
    # s3_bucket_name = 'edm-db-drift-detection-reports-'+os.getenv('environment')
    # s3_bucket_name = 'edm-db-drift-detection-reports-'+ 'sandbox'
    s3_bucket_name = 'edm-saas-dev-arrxi/2024-02-13/'
...
def run_query_using_secret (sql,parameter_list=None):
    # Function to query and RDS instance
    # Parameters
    #   service_client  - connection to the AWS secretmanager service
    #   arn - secrets ARN
    #   sql - sql to execute

    result = []
    # get connection to db with the current secret
    # conn = get_connection(get_secret_dict(service_client, arn, "AWSCURRENT"))
    conn = pgdb.connect(user = "admin", host = "localhost", port = 5432, dbname = "postgres")
    if conn:

        try:
            with conn.cursor() as cur:
                if parameter_list:
                    cur.execute(sql,parameter_list)
                else:
                    cur.execute(sql)
                if cur.rowcount > 0 :
                    result = cur.fetchall()
            ...
            conn.rollback()
            # logger.error("query_using_secrets: Execption encountered executing query %s." % e)
            # raise ValueError("query_using_secrets: Execption encountered executing query %s." % e)

        finally:
            conn.commit()
            conn.close()

        # logger.info("query_using_secrets: Successfully executed query in PostgreSQL DB using secret in %s." % arn)
        return result
    else:
        # logger.error("query_using_secrets: Unable to log into database with current secret of secret ARN %s" % arn)
...
if __name__ == "__main__":
    ...
    # reporting_db_secrets_client = boto3.client('secretsmanager', region_name = "us-east-2")
    # reporting_db_secret_arn = os.getenv("db_secret_arn") # need to input reporting db arn
    ...
    succesfully_connected_acct_ids = []
    ...
    for acct_id in acct_ids:
        ...
        while index < len(rds_secrets_list):
            ...
            if rds_secrets_list[index]['DBAdminSecretARN'] == "":
                summary_list.append({"ScanID":scan_id, "AccountID":acct_id, "RDSIdentifier": rds_secrets_list[index]['DBInstanceID'], "SecretARN":"NA", "Summary":"No Secret found for the R
DS in secret manager"})
            else:
                summary_list.append({"ScanID":scan_id, "AccountID":acct_id, "RDSIdentifier": rds_secrets_list[index]['DBInstanceID'], "SecretARN":rds_secrets_list[index]['DBAdminSecretARN'
], "Summary":"Secret Found for the rds in secret manager"})
            index = index +1
        ...
        while index < len(rds_secrets_list):
            if rds_secrets_list[index]['DBAdminSecretARN'] != "":
                ...
                if status_code < 0:
                    ...
                    summary_list.append({"ScanID":scan_id, "AccountID":acct_id, "RDSIdentifier": rds_secrets_list[index]['DBInstanceID'], "SecretARN":rds_secrets_list[index]['DBAdminSecret
ARN'], "Summary":"Unable to connect to the target Db with Secret"})

                #If query execution successfull runs the CIS sql scripts on target db for drifts.
                else:
                    print("connection successful")
                    succesful_connections += 1
                    succesfully_connected_acct_ids = map(lambda d: d['AccountID'], summary_list)
                    summary_list.append({"ScanID":scan_id, "AccountID":acct_id, "RDSIdentifier": rds_secrets_list[index]['DBInstanceID'], "SecretARN":rds_secrets_list[index]['DBAdminSecret
ARN'], "Summary":"Successfully Connected to db with Secret"})
...
s_sequences_table","roles_privs_fdw_table","roles_login_fdw_table","roles_privs_language_table","functions_ownership_roles_table"]
    ...
    generate_csv_and_pdf_reports_for_the_drift_tables(table_names_list, scan_id)

    print("Total number of successful connections: ", succesful_connections)
    print("Succesfully connected accounts: ", succesfully_connected_acct_ids)

    #calculate total execution time
    total_time_seconds = end_time - start_time
    total_time_minutes = total_time_seconds / 60
    print("Total execution time:", total_time_minutes, "minutes")
