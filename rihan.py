    ...
    if rds_summary_list == [] and len(rds_instances['DBInstances']) > 0:
        while index < len(rds_instances['DBInstances']):
            try:
                ...
                if 'DBClusterIdentifier' in rds_instances['DBInstances'][index]:
                    #If any error with summary list refer Comment above the summary_table_parameter_list
                    rds_summary_list.append({"ScanID":scan_id, "AccountID":acct_id, "AccountName":acct_ids_dict.get(acct_id), "RDSIdentifier": str(rds_instances['DBInstances'][index]['DBCl
usterIdentifier'].rsplit('-',1)[0]), "SecretARN":"NA", "Summary":error})
                else:
                    #If any error with summary list refer Comment above the summary_table_parameter_list
                    rds_summary_list.append({"ScanID":scan_id, "AccountID":acct_id, "AccountName":acct_ids_dict.get(acct_id), "RDSIdentifier": rds_instances['DBInstances'][index]['DBInstan
ceIdentifier'], "SecretARN":"NA", "Summary":error})
...
def get_account_ids_from_s3():
    error = ""
    acct_ids = []
    acct_ids_dict = {}

    try:
        print("HAAHAHAHAHAHHHA1")
        s3 = boto3.resource('s3')
        s3.meta.client.download_file('edm-saas-dev-arrxi', '2024-03-12/results.json', 'results1.json')
        # Open the JSON file
        ...
        filtered_data = [{"account_id": key, "entries": value} for key, value in data.items() if value and any(entry.get("Engine") == "aurora-postgresql" and any(tag["Key"] == "tier" and t
ag["Value"] in ["1", "2", "3", "6"] for tag in entry.get("account_tags", [])) for entry in value)]
        ...
        error = f"""Exception Caught - Error = {client_error}"""
        print("exception%% @@@@@@@")
        print("HAHAHAHAHAAHAH3")
        print(client_error)
        print("HAHAHAHAHAHHA4")
...
def get_account_ids():
    ...
    #sts_org_arn = "arn:aws:iam::009973789139:role/edm/LambdaCrossAccountFunctionRole"
    acct_ids = []
    acct_ids_dict = {}
    try:
...
def get_roles_specific_privileges_sequences_list(scan_id, acct_id, roles_specific_privileges_sequences_list, result) :
    ...
    for each in result:
        ...
        if each_dict.get("rolname") == None:

            roles_specific_privileges_sequences_list.append({
                'ScanId':scan_id,
                'AccountID': acct_id,
                'rolname':'None',        # Fixed by Sanjeeb
                'database_name':each_dict.get("database_name"),
                'object_type':each_dict.get("object_type"),
                'object_name':each_dict.get("object_name"),
                'privilege_type':each_dict.get("privilege_type"),
                'privileges':",".join(each_dict.get("privileges")), # Fixed by Sanjeeb
...
def generate_csv_and_pdf_reports_for_the_drift_tables(secrets_client, reporting_db_secret_arn, table_names_list, scan_id):
    ...
    default_parameters = {
        ...
        "instances_info":["id","recordedtime","scanid","accountid","dbinstanceid","secrethost","secretport","multiaz","engine","EngineVersion","cluster_encrypted","database_name","BackupRe
tentionPeriod","PubliclyAccessible","instance_encrypted"],
        "snapshots_info":["id","recordedtime","scanid","accountid","dbinstanceid","dbclusteridentifier","DBSnapshotIdentifier_Instance_or_Cluster","DBSnapshotEncrypted_Instance_or_Cluster"
,"DBSnapshotType_Instance_or_Cluster"],
        "user_roles":["id","recordedtime","scanid","accountid","user_role_id","user_role_name","role_can_login","other_role_id","other_role_name","has_replication_perm","has_createrole_per
m","has_createdb_perm"],
...
def lambda_handler():
    ...
    #acct_ids_dict = get_account_ids()
    acct_ids_dict, error = get_account_ids_from_s3()
    ...
    for acct_id in acct_ids:
        ...
        try:
            ...
            #If any error with summary list refer Comment above the summary_table_parameter_list
            summary_list.append({"ScanID":scan_id, "AccountID":acct_id, "AccountName":acct_ids_dict.get(acct_id), "RDSIdentifier": "NA", "SecretARN":"NA", "Summary":"Assume role connection
 successful"})
        except ClientError as client_error:
            #If any error with summary list refer Comment above the summary_table_parameter_list
            summary_list.append({"ScanID":scan_id, "AccountID":acct_id, "AccountName":acct_ids_dict.get(acct_id), "RDSIdentifier": "NA", "SecretARN":"NA", "Summary":"Error Assuming the rol
e on target account"})
        ...
        while index < len(rds_secrets_list):
            ...
            if rds_secrets_list[index]['DBAdminSecretARN'] == "":
                summary_list.append({"ScanID":scan_id, "AccountID":acct_id, "AccountName":acct_ids_dict.get(acct_id), "RDSIdentifier": rds_secrets_list[index]['DBInstanceID'], "SecretARN":
"NA", "Summary":"No Secret found for the RDS in secret manager"})
            else:
                summary_list.append({"ScanID":scan_id, "AccountID":acct_id, "AccountName":acct_ids_dict.get(acct_id), "RDSIdentifier": rds_secrets_list[index]['DBInstanceID'], "SecretARN":
rds_secrets_list[index]['DBAdminSecretARN'], "Summary":"Secret Found for the rds in secret manager"})
            index = index +1
        ...
        while index < len(rds_secrets_list):
            if rds_secrets_list[index]['DBAdminSecretARN'] != "":
                ...
                if status_code < 0:
                    ...
                    summary_list.append({"ScanID":scan_id, "AccountID":acct_id, "AccountName":acct_ids_dict.get(acct_id), "RDSIdentifier": rds_secrets_list[index]['DBInstanceID'], "SecretA
RN":rds_secrets_list[index]['DBAdminSecretARN'], "Summary":"Unable to connect to the target Db with Secret"})

                #If query execution successfull runs the CIS sql scripts on target db for drifts.
                else:
                    print("connection successful")
                    summary_list.append({"ScanID":scan_id, "AccountID":acct_id, "AccountName":acct_ids_dict.get(acct_id), "RDSIdentifier": rds_secrets_list[index]['DBInstanceID'], "SecretA
RN":rds_secrets_list[index]['DBAdminSecretARN'], "Summary":"Successfully Connected to db with Secret"})
                    ...
                    audit_roles_list = get_audit_roles_list(scan_id, acct_id, rds_secrets_list[index]['DBInstanceID'], audit_roles_list, result, dbname=rds_secrets_list[index]['DatabaseNam
e'])
...
e'])
                    ...
                    """
                    result = pgs.run_query_using_secrets(secrets_client, rds_secrets_list[index]['DBAdminSecretARN'], sql)
                    public_roles_list = get_public_roles_list(scan_id, acct_id, rds_secrets_list[index]['DBInstanceID'], public_roles_list, result, dbname=rds_secrets_list[index]['Database
Name'], query_number=1)
...
Name'], query_number=8)
    ...
    table_names_list = ["public_role_privileges", "audit_role_privileges",summary_table_name,"instances_info","snapshots_info","user_roles","database_permissions","schema_privs_role","role
_specific_privs","role_priv_tables","role_specific_priv_tables","views_ownership_usage_privs_table","view_privs_role_table","sequence_ownership_usage_privs_table","roles_specific_privilege
s_sequences_table","roles_privs_fdw_table","roles_login_fdw_table","roles_privs_language_table","functions_ownership_roles_table"]
