...
def get_others_roles_list(scan_id, acct_id, rds_identifier, others_roles_list, result, dbname):
    #Parse the sqlquery output and add to the list as dict for updating the audit roles table.
    for each in result:
        each_dict = each._asdict()
        others_roles_list.append({
            'ScanId':scan_id,
            'AccountID': acct_id,
            'DbIdentifier': rds_identifier,
            'Dbname':dbname,
            'UserRoleId':each_dict.get("user_role_id"),
            'UserRoleName':each_dict.get("user_role_name", "NULL"),
            'OtherRoleId':each_dict.get("other_role_id", "ALL"),
            'OtherRoleName':each_dict.get("other_role_name", "ALL")
        })
    return others_roles_list
...
def lambda_handler(event, context):
    ...
    for acct_id in acct_ids_dict.keys():
        ...
        while index < len(rds_secrets_list):
            ...
            if rds_secrets_list[index]['DBAdminSecretARN'] != "":
                ...
                #If query execution successfull runs the CIS sql scripts on target db for drifts.
                    ...
                    sql = f"""
                    select a.oid as user_role_id,, a.rolname as user_role_name, b.roleid as other_role_id, c.rolname as other_role_name from pg_roles a,inner join pg_auth_members b on a.oid=b.member inner join pg_roles c on b.roleid=c.oid;
                    """
                    result = pgs.run_query_using_secrets(secrets_client, rds_secrets_list[index]['DBAdminSecretARN'], sql)
                    others_roles_list = get_others_roles_list(scan_id, acct_id, rds_secrets_list[index]['DBInstanceID'], audit_roles_list, result, dbname=rds_secrets_list[index]['DatabaseName'])
        ...
        if others_roles_list:
            flag = 3
            parameter_list = get_others_roles_list[0].keys()
            #Creates or update Others role privileges table
            create_or_alter_table(parameter_list, table_name="others_role_privileges")
            update_table(others_roles_list, parameter_list, table_name='others_role_privileges')
            

    #updates summary table
    update_table(summary_list, summary_table_parameter_list, summary_table_name)
    if flag == 3:
        table_names_list = ["others_role_privileges", "public_role_privileges","audit_role_privileges",summary_table_name,"instances_info","snapshots_info"]
    if flag == 2:
