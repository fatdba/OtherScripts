...
def get_user_roles_list(scan_id, acct_id, result):
...
def lambda_handler(event, context):
        ...
        while index < len(rds_secrets_list):
            if rds_secrets_list[index]['DBAdminSecretARN'] != "":
                ...
                #If query execution successfull runs the CIS sql scripts on target db for drifts.
                    ...
                    """
                    result = pgs.run_query_using_secrets(secrets_client, rds_secrets_list[index]['DBAdminSecretARN'], sql)
                    user_roles_list = get_user_roles_list(scan_id, acct_id, result)
                    if result:
    ...
    if flag == 2:
        table_names_list = ["public_role_privileges", "audit_role_privileges",summary_table_name,"instances_info","snapshots_info","user_roles"]
    elif flag == 1:
        table_names_list = ["audit_role_privileges",summary_table_name,"instances_info","snapshots_info","user_roles"]
    else:
        table_names_list = [summary_table_name,"instances_info","snapshots_info","user_roles"]
